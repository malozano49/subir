/**
 *  @file        : opac-reader-loans-history.init.js
 *
 *  @description : js de aplicación en la página Préstamos y solicitudes -> Histórico
 *  @license     : baratz
 *  @copyright   : 2019
 *
...
/**
 *  Contenedor de acciones para la página Préstamos y solicitudes -> Histórico
 *
 *  @cont READER_LOANS_HISTORY
 *
 */
const READER_LOANS_HISTORY={
  /**
   *  Inicialización
   *
   *  @method READER_LOANS_HISTORY.init
   */
  init:()=>{
    READER_LOANS_HISTORY.load_plugins();
  },
  /**
   *  Carga e inicializacion de plugins
   *
   *  @method READER_LOANS_HISTORY.load_plugins
   */
  load_plugins:()=>{
    const listOfJS=[
    ...
    $.when(control).done(()=>{
      READER_LOANS_HISTORY.secuencia_ejecucion();
    });
  },
  /**
   *  Secuencia de ejecucion de scripts
   *
   *  @method READER_LOANS_HISTORY.secuencia_ejecucion
   */
  secuencia_ejecucion:()=>{
