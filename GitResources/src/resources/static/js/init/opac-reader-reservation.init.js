/**
 *  @file        : opac-reader-reservation.init.js
 *
 *  @description : js de aplicación en la página Préstamos y solicitudes -> Mis solicitudes
 *  @license     : baratz
 *  @copyright   : 2019
 *
...
/**
 *  Contenedor de acciones para la página Préstamos y solicitudes -> Mis solicitudes
 *
 *  @cont READER_RESERVATION
 *
 */
const READER_RESERVATION={
  /**
   *  Inicialización
   *
   *  @method READER_RESERVATION.init
   */
  init:()=>{
    READER_RESERVATION.load_plugins();
  },
  /**
   *  Carga e inicializacion de plugins
   *
   *  @method READER_RESERVATION.load_plugins
   */
  load_plugins:()=>{
    const listOfJS=[
    ...
    $.when(control).done(()=>{
      READER_RESERVATION.secuencia_ejecucion();
    });
  },
   ...
   *
   *  @method READER_RESERVATION.secuencia_ejecucion
   */
  secuencia_ejecucion:()=>{
